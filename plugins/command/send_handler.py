...
async def send_with_pic_handler(self, client: Client, msg: types.Message, key: str, hastag: list):
...
async def send_menfess_handler(self, client: Client, msg: types.Message, link: str = None):
    helper = Helper(client, msg)
    db = Database(msg.from_user.id)
    ...
    # Check if the message mentions the sender's username
        self.username = "-" if not self.msg.from_user.username else '@' + self.msg.from_user.username    
    if not self.username in msg.text.lower():
        return await msg.reply('⚠️ pesan gagal terkirim, anda menggunakan username orang lain, untuk mencegah fake promote silahkan gunakan username anda sendiri atau buat pesan promote tanpa username = (menfess)', quote=True)
